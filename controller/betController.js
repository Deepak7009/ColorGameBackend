...
const getLowestBetNumber = async (req, res) => {
  try {
    ...
    const leastTotalAmountNumber = numbers.reduce((minIndex, currentAmount, currentIndex, array) => {
      return currentAmount < array[minIndex] ? currentIndex : minIndex;
    }, 0);

    // Return the number with the least totalAmount
    res.status(200).json({
...
const getWinningBets = async (req, res) => {
  try {
    ...
    // Find winning bets based on either the result number or associated color
