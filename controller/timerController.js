...
const startTimer = async (req, res) => {
    try {
      ...
      let timer = await Timer.findOne({ periodId: newPeriodId });
      if (!timer) {
        timer = new Timer({ periodId: newPeriodId });
        await timer.save();
      }
  
      // Function to handle the 30-second countdown and create a new entry
      const startCountdown = async () => {
        setTimeout(async () => {
          // Increment periodId by 1
          timer.periodId++;
          await timer.save();
          console.log(`Period ID ${timer.periodId} has been incremented.`);
  
          // Create a new Timer document
          const newTimer = new Timer({ periodId: timer.periodId });
          await newTimer.save();
          console.log(`New Timer document created with period ID ${newTimer.periodId}.`);
  
          // Restart the countdown and create a new entry
          await startCountdown();
        }, 30 * 1000); // 30 seconds in milliseconds
      };
  
      // Start the initial countdown and create a new entry
      await startCountdown();
